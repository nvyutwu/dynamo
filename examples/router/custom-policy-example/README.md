...
## Opt-in decision explanations

The examples inherit `WorkerPicker::pick_with_explanation`, which calls their existing picker once and reports `picker_does_not_supply_explanation` when `DYN_ROUTER_DECISION_TRACE_ENABLED=1`. This preserves each example's ordinary selection behavior. A policy that implements this callback must return evidence from the same immutable candidate rows used for its selection; the host validates the selected row before recording the envelope. The shipped two-tier policy demonstrates rule evidence without inventing additive costs. See the [custom worker-selection contract](https://github.com/ai-dynamo/dynamo/blob/main/docs/fern/pages/developer-guide/knowledge-base/modular-components/router/custom-worker-selection.mdx#decision-time-explanations) for fields and missing-data semantics.
