...
mod tests {
    ...
    // Kimi K3 dynamic tools live on `messages[].tools`, not the top-level list.
    // The HTTP parsing options must still see them, or a call to a dynamically
    // declared tool is stripped from the response.
    #[test]
