...
pub struct AuditPolicy {
    ...
    /// Lowercased denylist of HTTP request header names to EXCLUDE from the
    /// audit record. Empty = capture all headers. Applies to every sink.
    pub header_exclude_list: Vec<String>,
}
...
fn load_from_env() -> AuditPolicy {
    ...
    let header_exclude_list = std::env::var(env_audit::DYN_AUDIT_HTTP_HEADER_EXCLUDE_LIST)
        .ok()
        .map(|raw| {
