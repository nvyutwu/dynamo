...
pub struct RouterRequestMetrics {
    ...
    pub raw_cache_rejected_candidates_total: prometheus::IntCounter,
    ...
    pub fn from_component(component: &Component) -> Arc<Self> {
        ROUTER_REQUEST_METRICS
            .get_or_init(|| {
                ...
                let raw_cache_rejected_candidates_total = metrics
                    .create_intcounter(
                        &router_metric("raw_cache_rejected_candidates_total"),
                        "Malformed candidates rejected by raw router-index coverage scans",
                        extra_labels,
                    )
                    .expect("failed to create router_raw_cache_rejected_candidates_total");
