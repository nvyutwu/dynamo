...
fn py_err_to_dynamo(err: PyErr) -> DynamoError {
    let (backend, message) = Python::with_gil(|py| {
        ...
        // http::service::error::SanitizedError::for_backend_status.
        let type_name = err
            .get_type(py)
            .getattr("__name__")
            .ok()
        ...
        if type_name.ends_with("ValidationError") {
            let message = err
                ...
                .unwrap_or_default();
            let json_msg = serde_json::json!({ "message": message, "code": 400 }).to_string();
            return (BackendError::InvalidArgument, json_msg);
        }
