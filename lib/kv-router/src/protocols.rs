...
fn hash_block_no_mm(chunk: &[u32], seed: u64, scratch_bytes: &mut Vec<u8>) -> LocalBlockHash {
    ...
    #[cfg(not(target_endian = "little"))]
    {
        ...
        return LocalBlockHash(xxh3::xxh3_64_with_seed(scratch_bytes, seed));
    }
}
...
pub fn compute_block_hash_for_seq(
    ...
    let block_size = kv_block_size as usize;
    let mut hashes = Vec::with_capacity(tokens.len() / block_size);
    let mut bytes = Vec::with_capacity(block_size * std::mem::size_of::<u32>());
    let mut mm_hashes = Vec::new();

    for (block_idx, chunk) in tokens.chunks_exact(block_size).enumerate() {
