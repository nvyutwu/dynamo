...
pub(super) fn oracle_cached_entry<C: WorkerConfigLike>(
    ...
    request
        ...
        .iter()
        .filter(|(worker, _)| eligibility.validate_worker_rank(workers, **worker).is_ok())
...
> SchedulerQueueActor<P, C, Sel, RF>
{
    ...
    fn select_worker_for_request(
...

        {
            ...
            let mut resident_eligibility = request
                .eligibility()
                .with_available_workers(available_worker_ids.as_deref());
            if self.selector.uses_exclusive_affinity_target()
                && let Some(target) = request.affinity_target
            ...
            let resident_oracle_cached_tokens =
                oracle_cached_entry(&workers, request, resident_eligibility)
...
mod tests {
    ...
    async fn cache_oracle_follows_custom_selection_without_changing_its_choice() {
        ...
        // Make the custom policy's choice independent of cache-aware prefill
        // cost so this test deliberately chooses less than the cache oracle.
        request.track_prefill_tokens = false;
        request
            .overlap
            .effective_cached_tokens
            .extend([(cold, 16), (warm, 64)]);
...
"#,
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn raw_coverage_affinity_fallback_keeps_resident_superset() {
        // Exercise the real queue and production selector, not a hand-built pair
        // of eligibility predicates. Overload must release affinity for both
        // the selected candidate universe and its raw resident oracle.
        for overloaded in [false, true] {
            let provider: OverloadedWorkerProvider = Arc::new(move || {
                Some(if overloaded {
                    HashSet::from([0])
                } else {
                    HashSet::new()
            ...
            let (queue, _slots) = make_queue_with_providers(2, 16, 64, Some(provider), None);
            let (mut request, rx) = make_request("affinity-raw-review", 64);
            let a = WorkerWithDpRank::new(0, 0);
            let b = WorkerWithDpRank::new(1, 0);
            request.affinity_target = Some(a.into());
            request.overlap.raw_index_state = super::super::RawIndexState::Observed;
            request
                .overlap
                .tier_overlap_blocks
                .device
            ...
            let expected = if overloaded { b } else { a };
            let expected_tokens = if overloaded { 48 } else { 16 };
            assert_eq!(response.best_worker, expected);
            let coverage = response.raw_cache_coverage;
            assert_eq!(
                coverage.observation,
                super::super::RawCacheObservation::Complete
            );
            for candidate in [coverage.resident, coverage.eligible, coverage.selected] {
                let candidate = candidate.unwrap();
                assert_eq!(candidate.worker_id, expected.worker_id);
                assert_eq!(candidate.total_tokens, expected_tokens);
            }
            assert_eq!(coverage.overload_gap_tokens, Some(0));
            assert_eq!(coverage.selection_gap_tokens, Some(0));
        }
    }
