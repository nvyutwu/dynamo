...
#[test]
fn positional_all_blocks_clear_does_not_guess_tier_names_from_ownership() {
    for ownership in ["GPU", "CPU"] {
        let event: RawKvEvent =
            from_slice(&to_vec(&("AllBlocksCleared", ownership)).unwrap()).unwrap();
        let RawKvEvent::AllBlocksCleared {
            medium,
            ownership: decoded_ownership,
        } = event
        else {
            panic!("expected AllBlocksCleared");
        };
        assert_eq!(medium, None, "ownership must not be guessed as medium");
        assert_eq!(decoded_ownership.as_deref(), Some(ownership));

        let event: RawKvEvent =
            from_slice(&to_vec(&("AllBlocksCleared", ownership)).unwrap()).unwrap();
        assert_eq!(
            ZmqEventNormalizer::new(2)
                .preprocess_with_reason(event, WorkerWithDpRank::new(7, 0))
                .unwrap_err(),
            ZmqEventFilterReason::UnknownOwnership,
            "colliding ownership remains unsupported and must fail closed"
        );
    }
}
