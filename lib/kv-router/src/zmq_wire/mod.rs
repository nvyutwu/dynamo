...
pub use convert::{
    StoredBlockOptions, convert_event, convert_event_with_partial_tail,
    create_stored_block_from_parts, create_stored_blocks, normalize_mm_placeholder_runs,
...
impl ZmqEventNormalizer {
    ...
    fn propagate_cache_namespace(
        ...
        match raw {
            ...
            RawKvEvent::AllBlocksCleared { .. } => {
                // The only supported AllBlocksCleared semantics are the legacy
                // broad reset. An explicit medium is rejected before this
                // method, and scoped resets use TierBlocksCleared.
                self.cache_namespaces
