...
<ParamField path="dynamo_component_router_input_f2_total" type="counter">
  ...
  <Note>The numbering skips `f1` on purpose. The router and the indexer cannot evaluate `f1`, so no such counter is emitted here. Do not reuse the name for something else.</Note>
</ParamField>
