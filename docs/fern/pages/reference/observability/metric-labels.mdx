...
<ParamField path="worker_type" type="string">
  `[Shared]` On `dynamo_frontend_worker_*`, `dynamo_frontend_router_queue_*`, and the `dynamo_component_router_*` decision counters — the worker role. In disaggregated mode both values appear; in aggregated mode all workers report as `decode`, so the `prefill` series stays at zero.
