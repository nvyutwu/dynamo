...
`input_sequence_hashes` are Dynamo's sequence-aware rolling hashes. `output_sequence_hashes`
continues the same chain across generated tokens. When an input ends mid-block, its first output hash
replaces the trailing input hash; otherwise output hashes append to the input chain. DynoSim maps
these hashes to compact internal IDs while loading the original trace; it does not create an
intermediate Mooncake file.
