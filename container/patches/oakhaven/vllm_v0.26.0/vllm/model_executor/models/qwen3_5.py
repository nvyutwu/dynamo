...
class Qwen3_5ForCausalLMBase(
    nn.Module,
    HasInnerState,
    # Mark the text-only causal LM as hybrid so is_hybrid is True for a bare
    # Qwen3_5MoeForCausalLM checkpoint; without it the model loads but the
    # Mamba/GDN align + hybrid-cache path never triggers. The class already
    # implements the required get_mamba_state_{shape,dtype}_from_config.
    IsHybrid,
    SupportsEagle3,
    SupportsLoRA,
    SupportsPP,
    ...
    # Hybrid (GDN/mamba) state classmethods — required by vLLM's _align_hybrid_block_size
    # (platforms/interface.py). Present on the ConditionalGeneration path but were MISSING on the
    # text CausalLM path -> AttributeError at KV-block alignment. Same impl (uses hf_text_config,
    # which resolves to the flat config for the text-only checkpoint).
    @classmethod
