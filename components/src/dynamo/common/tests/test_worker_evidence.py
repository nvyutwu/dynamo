...
@pytest.mark.parametrize("value", ["2", "enabled", "sometimes"])
def test_invalid_enable_value_disables_collector_without_spawning(
    monkeypatch, caplog, value
):
    monkeypatch.setenv(worker_evidence.ENABLED_ENV, value)
    calls = []
    monkeypatch.setattr(
        worker_evidence.subprocess, "Popen", lambda *a, **kw: calls.append(a)
    )

    assert worker_evidence._enabled(value) is False
    assert worker_evidence.start_worker_evidence_collector() is None
    assert calls == []
    assert worker_evidence.ENABLED_ENV in caplog.text
    assert repr(value) in caplog.text


@pytest.mark.parametrize(
    "error", [OSError("launch failed"), ValueError("invalid launch")]
)
def test_collector_launch_failure_does_not_block_worker(monkeypatch, caplog, error):
    monkeypatch.setenv(worker_evidence.ENABLED_ENV, "true")
    monkeypatch.setattr(worker_evidence, "_cleanup_registered", False)

    def fail_launch(*args, **kwargs):
        raise error

    monkeypatch.setattr(worker_evidence.subprocess, "Popen", fail_launch)
    assert worker_evidence.start_worker_evidence_collector() is None
    assert worker_evidence._collector_process is None
    assert worker_evidence._collector_owner_pid is None
    assert not worker_evidence._cleanup_registered
