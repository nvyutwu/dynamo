...
def _enabled(value):
    ...
    if normalized in ("", "0", "false", "no", "off"):
        return False
    logger.warning(
        "%s=%r is not a recognized boolean; worker evidence collector disabled",
        ENABLED_ENV,
        value,
    )
    return False
...
def start_worker_evidence_collector():
    ...
    except Exception:  # An optional diagnostic must not prevent worker startup.
        logger.warning(
            "Worker evidence collector launch failed; disabled", exc_info=True
        )
        return None
